...
import edu.uci.ics.crawler4j.util.FileUtils;
...
public class ImageCrawler extends WebCrawler
{
    ...
    public static void configure(final String[] domain, final String storageFolderName) {
	...
	for ( final String domain : crawlDomains ) {
	...
	final String hashedName = Cryptography.MD5( url ) + extension;

	// store image
	final String path = storageFolder.getAbsolutePath() + FileUtils.FILE_SEPARATOR + hashedName;
	try {
	    Files.write( page.getContentData(), new File( path ) );
	} catch ( final IOException e ) {
	    logger.error( String.format( "Exception while trying to store data at %s", path ) );
	    e.printStackTrace();
	}
